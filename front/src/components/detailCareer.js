...
export default class detailCareer extends Component {
	...
	componentDidMount() {
		...
		if (!navigator.onLine) {
			...
			if (token) {
				fetch(
					`https://futureguide.herokuapp.com/carrera/${nombre.toUpperCase()}/${name.toUpperCase()}`,
					{
						method: 'GET',
						headers: new Headers({
							Authorization: token
			...
			Swal.mixin({
				...
				.then((result) => {
					if (result.value) {
						...
						fetch(
							`https://futureguide.herokuapp.com/carrera/${this.state.universidad.toUpperCase()}/${this.state.programa.toUpperCase()}/${'comentarios'}`,
							{
								method: 'POST',
								headers: new Headers({
									Authorization: token,
							...
							fetch(
								`https://futureguide.herokuapp.com/usuarios/${Cookies.get(
									'USERNAME'
								)}/${'comentarios'}`,
								{
									method: 'POST',
		...
		return (
			<div className='container-fluid detailCareer' role='main'>
				<div className='row'>
					<div className='col-lg-7 col-xl-7 col-md-7 col-12 infoDetail'>
						<div className='row'>
							<div className='col-lg-6 col-xl-6 col-md-6 col-12'>
								...
								{this.state.altaCalidad ? (
									<span className='badge badge-calite'>
										<FormattedMessage id='highQuality' />
									</span>
								) : (
									false
								)}
								...
								<strong className='atributosCareer'>
							...
							<ul className='col-lg-6 col-xl-6 col-md-6 col-12x   list-group list-group-flush'>
								<li className='list-group-item  d-flex justify-content-between align-items-center'>
									<img
										src={timeImage}
										className='img-fluid img-responsive img-Little'
										alt='Duracion de la carrera'
									/>
									<strong className='atributosCareer'>
										{this.state.duracion}{' '}
									</strong>{' '}
									<strong className=' cursiveAnotation text-right'>
										<FormattedMessage id='semesters' />
									</strong>
								</li>
								<li
								...
								>
									...
									<p className='atributosCareer'>
										<FormattedNumber value={this.state.salario} />
									</p>{' '}
									<strong className=' cursiveAnotation text-right'>
										<FormattedMessage id='inAverage' />
									</strong>
								</li>
								{this.state.acreditacionInternacional ? (
									<li className='list-group-item  d-flex justify-content-between align-items-center'>
										<img
											src={acreditacionInternacionalImg}
											className='img-fluid img-responsive img-Little'
											alt='Duracion de la carrera'
										/>
										<p className='atributosCareer' id='acreditacionInt'>
											{this.state.acreditacionInternacional}
										</p>
									</li>
								) : (
							...
							<div className='col-12 overflow'>
								...
								>
									...
									<div
										...
										role='listbox'
										aria-label='carousel de reseñas'
									>
											...
											>
												<div className='card'>
													<div className='card-header'>
														<h3 className='tituloComentario'>
															{this.state.comentarios[0].titulo}
														</h3>
													</div>
													<div className='card-body'>
														<blockquote className='blockquote mb-0'>
															<p className='comentarioDescripcion'>
																"{this.state.comentarios[0].descripcion}"
															</p>
															{this.state.comentarios[0].recomendada ? (
																<span className='badge badge-style badge-recomendada'>
																	<FormattedMessage id='recommended' />
																</span>
															) : (
																<span className='badge badge-style badge-nrecomendada'>
																	<FormattedMessage id='notRecommended' />
																</span>
															)}
														</blockquote>
													</div>
										...
										{this.state.comentarios.map((el, i) => (
						...
						<div className='row boton'>
							<div className='col-12 text-center'>
								{this.state.onLine ? (
									<button
										type='button'
										className='btn btnNewComment'
										onClick={this.reseña}
									>
										<FormattedMessage id='newComment' />
									</button>
								) : (
									<button
										type='button'
										...
										data-placement='bottom'
										data-html='true'
										title=' <em>No puedes crear reseñas sin internet</em>'
										disabled
									>
										<FormattedMessage id='newComment' />{' '}
									</button>
					...
					<div
						className='col-5 d-none d-md-block text-center'
						id='videosColumn'
					>
						<h2>
						...
						<div className='scrollbar scrollbar-videos' tabIndex='0'>
							{this.state.videos.map((element, index) => (
								<div
									key={index}
									className='embed-responsive embed-responsive-16by9 videos'
								>
									<iframe
