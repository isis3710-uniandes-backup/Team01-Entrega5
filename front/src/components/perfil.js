...
export default class perfil extends Component {
			...
			comentarios: [
				{
					titulo: 'Reseña 1',
					descripcion:
						'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eligendi, eos explicabo fugit,\n' +
	...
	componentDidMount() {
		...
		if (token) {
			fetch(
				'https://futureguide.herokuapp.com/usuarios/' + this.state.username,
				{
					method: 'GET',
					headers: new Headers({
						Authorization: token
				...
				.then((json) => {
					this.setState({
						...
						email: json['correo'],
						comentarios:
							json['comentarios'] === undefined ? [] : json['comentarios']
					});
				});
		}
	}
					...
					<div className='row no-gutters'>
						<div className='col-9'>
							<div className='card-body perfil_body'>
								<h1 className='card-title text-center perfil_title'>
									{this.state.username}
								</h1>
								<div className='row'>
									<div className='col-6'>
										<div className='col-12'>
											<p className='card-text text-center perfil_text1'>
												<b>
										...
										<div className='col-12'>
											<p className='card-text text-center perfil_text2'>
												{this.state.name}
											</p>
										</div>
									</div>
						...
						<div className='col-3 text-center'>
							<img
								...
								className='card-img rounded-circle perfil_img'
								alt='Imagen Aleatoria de Perfil'
							></img>
						</div>
					</div>
				</div>
					...
					{this.state.comentarios.length !== 0 ? (
						<div className='row'>
							<div className='col-2 boton-comentario'>
								<div id='list-example' className='list-group'>
									{this.state.comentarios.map((e, index) => (
										...
										>
											<FormattedMessage id='review' /> {index + 1}
										</a>
									))}
								</div>
							</div>
							<div
								...
								data-offset='0'
								className='col-10 comentario scrollbar-primary scrollspy-example '
								tabIndex='0'
							>
								{this.state.comentarios.map((e, i) => (
									<div className='col-12 info-comentario' key={i}>
										<h1 className='reseña-title' id={'list-item-' + i}>
											<b>{e['titulo']}</b>
										</h1>
										<p>{e['descripcion']}</p>
									</div>
