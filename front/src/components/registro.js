...
export default class register extends Component {
	...
	changeValue(e) {
		if (e.target.id === 'username') {
				...
				() => {
					if (this.state.usernameError === null) {
						if (this.state.username.length < 6) {
							this.setState({
								usernameError:
									'Su nombre de usuario debe tener más de 6 caracteres'
							});
						}
					}
					if (this.state.username.length > 6) {
						fetch(
							'https://futureguide.herokuapp.com/usuarios/' +
								this.state.username,
							{
								method: 'GET'
							}
			...
			if (e.target.id !== '') checkEmail = true;
				...
				() => {
					if (this.state.passwordError === null) {
						if (
							...
							this.setState({
								passwordError:
									'Tu contraseña debe contener por lo menos un caracter numérico y una mayúscula.'
							});
						}
					}
				}
			);
			if (e.target.id !== '') checkPassword = true;
		}
	}
	signUp() {
		try {
			...
			if (this.state.username !== '' && this.state.usernameError === null)
				checkUsername = true;
			if (this.state.name !== '') checkName = true;
					...
					fetch('https://futureguide.herokuapp.com/register', {
						...
						.then((res) => {
							...
							if (res.status === 200) {
								...
								toast.error(
									'Lo sentimos, algo fallo en el servidor. Vuelve a intentarlo.',
									{
										containerId: 'A',
										position: 'top-center',
										autoClose: 1500,
